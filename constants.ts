type airWeather = {
    aqi: number,
    temp: number,
    humidity: number
} 
type waterWeather = {
    ph: number,
    temp: number,
    turbuidity: number
} 
export type weatherData = {
    airWeather: airWeather,
    waterWeather: waterWeather
}
type Booking = {
    bus_url: string,
    train_url: string,
    cab_url: string
}
type Location = {
    img: string,
    name: string,
    desc: string,
    map_url: string
}
type Hotel = {
    name: string,
    address: string,
    booking_url: string,
    img: string
}

type Restorant = {
    name: string,
    img: string,
    type: string,
    map_url: string
}
export type RiverData = {
    name: string,
    desc: string,
    background_url: string,
    air_weather: airWeather,
    water_weather: waterWeather,
    booking: Booking,
    locations: Location[],
    hotels: Hotel[],
    restorants: Restorant[]
}
export interface RiverProps {
    name: string,
    desc: string,
    background_url: string,
    air_weather: airWeather,
    water_weather: waterWeather,
    booking: Booking,
    locations: Location[],
    hotels: Hotel[],
    restorants: Restorant[]

}


export const GangaRiverData: RiverData = {
    name: "Ganga River(Haridwar)",
    desc: " The Ganga River, revered as the lifeline of India and one of the holiest rivers in Hinduism, holds deep cultural and spiritual significance in Ayodhya. While Ayodhya is renowned as the birthplace of Lord Rama and is associated with the Saryu River, the presence of the Ganga in this sacred city adds another layer of sanctity.",
    background_url: "https://images.unsplash.com/photo-1511754863001-18d44abd0a93?w=500&auto=format&fit=crop&q=60&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8aGFyaWR3YXJ8ZW58MHx8MHx8fDA%3D",
    air_weather: {
        aqi: 203,
        temp: 40,
        humidity: 45
    },
    water_weather: {
        ph: 8,
        temp: 40,
        turbuidity: 78
    },
    booking: {
        bus_url: "https://www.redbus.in/",
        train_url: "https://www.irctc.co.in/nget/train-search",
        cab_url: "https://www.olacabs.com/"
    },
    locations: [{
        img: "https://media.gettyimages.com/id/169546647/photo/har-ki-pauri-haridwar-india.jpg?s=612x612&w=0&k=20&c=TzwEjTTH1Ela4Pkk2OopbYta_0knQSF0Yum73d-W_vc=",
        name: "Har Ki Pauri",
        desc: " A sacred ghat where the Ganges leaves the mountains and enters the plains, hosting the evening Ganga Aarti. ",
        map_url: "https://maps.app.goo.gl/P5CduqHkxhEZqxQZA"
    },
    {
        img: "https://media.gettyimages.com/id/850747646/photo/panchkula-india-devotees-on-the-first-day-of-the-navratra-at-mansa-devi-temple-on-september-21.jpg?s=612x612&w=0&k=20&c=DllDoXSM5f6m3dksf85RbtmGuijETJKXzvSMGIJJ1Dg=",
        name: "Mansa Devi Temple",
        desc: " Situated atop Bilwa Parvat, this temple dedicated to Goddess Mansa Devi offers panoramic views of Haridwar.",
        map_url: "https://maps.app.goo.gl/e1jpmwBHXA5xjUVt6"
    },
    {
        img: "https://media.gettyimages.com/id/481688165/photo/pilgrims-at-a-temple-chandi-temple-haridwar-uttarakhand-india.jpg?s=612x612&w=0&k=20&c=KFs1fPNG_OYhS02j_4rboSK62abTdZH9hQPk7QbiuOo=",
        name: "Chandi Devi Temple",
        desc: " Located atop Neel Parvat, this temple dedicated to Goddess Chandi offers a scenic trek or a cable car ride.",
        map_url: "https://maps.app.goo.gl/7MdSM5mPqP7oi1B1A"
    }, {
        img: "https://media.gettyimages.com/id/1172742271/photo/german-monastery-interior-view.jpg?s=612x612&w=0&k=20&c=C7K5nKUqtcCGKXh1natbDCN2JbkPKtqmoxcVtWKADnU=",
        name: "Maya Devi Temple",
        desc: " One of the oldest temples in Haridwar, dedicated to Goddess Maya Devi, and a significant pilgrimage site.",
        map_url: "https://maps.app.goo.gl/VRoYXYDX22GqQBbHA"
    },
    {
        img: "https://media.gettyimages.com/id/520764838/photo/chilla-name-at-display-on-log-at-the-entrance-at-rajaji-national-park-uttarakhand-india.jpg?s=612x612&w=0&k=20&c=-y2UNV8A-z0G-boO0LBLlN4FagYYKWt5K-RzHToMA6g=",
        name: "Rajaji National Park",
        desc: " Located near Haridwar, this national park offers opportunities for wildlife safari and nature exploration.",
        map_url: "https://maps.app.goo.gl/M8xeVM2uAPYAFYjk8"
    },
    {
        img: "https://media.gettyimages.com/id/1210107506/photo/chandigarh-india-a-view-of-the-empty-grounds-at-shanti-kunj-park-during-lockdown-against.jpg?s=612x612&w=0&k=20&c=PHc32JIDgO8A2ph3afN2X36QS4UDw3uAW7PzlX8MU3Y=",
        name: " Shanti Kunj",
        desc: " A tranquil ashram and spiritual center known for its yoga and meditation teachings, promoting peace and spiritual growth.",
        map_url: "https://maps.app.goo.gl/uLuSDyt88eM2Mx617f"
    },],
    hotels: [{
        name: "Ganga Lahari",
        img: "https://cf.bstatic.com/xdata/images/hotel/max1024x768/502052458.jpg?k=4dafda2f4be9ac9188f2ee1f642f9614af3100f38315eff8bbece1787e2eb620&o=&hp=1",
        address: "Gau Ghat Har ki Pauri, 226443 Haridwār",
        booking_url: "https://www.booking.com/hotel/in/ganga-lahri.en-gb.html?aid=311984&label=ganga-lahri-PfuhjC96iKDMDW7JT4MyTgS266319318702%3Apl%3Ata%3Ap1%3Ap2%3Aac%3Aap%3Aneg%3Afi%3Atikwd-35612166781%3Alp9300710%3Ali%3Adec%3Adm%3Appccp%3DUmFuZG9tSVYkc2RlIyh9YXORK0YJiVoOxcWODxYDaAA&sid=70ee43153ae4fc9345999e4405dcf315&dest_id=-2097190;dest_type=city;dist=0;group_adults=2;group_children=0;hapos=1;hpos=1;no_rooms=1;req_adults=2;req_children=0;room1=A%2CA;sb_price_type=total;sr_order=popularity;srepoch=1712644773;srpvid=b2492db72e700193;type=total;ucfs=1&#hotelTmpl"
    }, {
        name: "Hotel Ganga Exotica",
        img: "data:image/jpeg;base64,",
        address: "Niranjani Akhara Road, near Safe Parking, Mayapur, Haridwar",
        booking_url: "https://www.agoda.com/en-in/hotel-ganga-exotica/hotel/haridwar-in.html?finalPriceView=1&isShowMobileAppPrice=false&cid=1841706&numberOfBedrooms=&familyMode=false&adults=1&children=0&rooms=1&maxRooms=0&checkIn=2024-04-18&isCalendarCallout=false&childAges=&numberOfGuest=0&missingChildAges=false&travellerType=-1&showReviewSubmissionEntry=false&currencyCode=INR&isFreeOccSearch=false&tag=461ebe2b-21d1-7ce2-d27e-969cba382470&isCityHaveAsq=false&los=1&searchrequestid=e0687092-b539-4fdb-8560-01bd0087d1bd&ds=QuGIk%2FBbCcjHwuDo"
    }, {
        name: "Hotel Ganga Azure",
        img: "https://q-xx.bstatic.com/xdata/images/hotel/max300/504330531.jpg?k=b82195fd5f12a2264093130414a802c07a19ccb31e2a78c94da89a41b685569d&o=&s=512x384",
        address: "Near Shiv Murti chowk Opp. Railway Station Haridwar",
        booking_url: "https://www.goibibo.com/hotels/ganga-azure-hotel-in-haridwar-8581916745583737264/?hquery={%22ci%22:%2220240416%22,%22co%22:%2220240417%22,%22r%22:%221-2-0%22,%22ibp%22:%22%22}&cc=IN&reviewType=gi&vcid=3545728938069300513"
    }],
    restorants: [{
        name: "Curry Leaf Haridwar",
        map_url: "https://www.google.com/maps/dir//16-A,+Ganga+Building,+Harki+Pauri+Haridwar,+249401/@25.5376752,83.1135899,12z/data=!4m8!4m7!1m0!1m5!1m1!1s0x3991df5c174001d3:0xdcf04a86c7df537e!2m2!1d83.1959914!2d25.5376982?entry=ttu",
        type: "Veg, Italian, South Indian, Chinese, Mexican, Japanese ",
        img: "https://lh3.googleusercontent.com/4XgK2YjGUm5sqPyoHRwnv83saKgymzDlJO4lHAXcQA87roodAJvodcLnkl3Bc3HDoLI1cXXu3Uw88SCWA_2Y0_VbziBzcggKRq4oezHKGA=w1200-rw"
    }, {
        name: "Sai Ganga Bhoj ",
        map_url: "https://www.google.com/maps/dir//shiv+vishram+grah,+Upper+Rd,+near+ganga+takies,+Har+Ki+Pauri,+Haridwar,+Uttarakhand+249401/@29.9550183,78.08641,12z/data=!4m8!4m7!1m0!1m5!1m1!1s0x39094795ce569861:0xce97a0fed412be5a!2m2!1d78.1688114!2d29.9550439?entry=ttu",
        type: " Veg, Italian, South Indian",
        img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSwoyHXda6D7AuS2GNTJO7a-p2KArOJ_hUfyA&s"
    }, {
        name: "Amber Hotel",
        map_url: "https://www.google.com/maps?s=web&rlz=1C1CHBD_enIN1025IN1025&sca_esv=eae1723b12d3aedd&sca_upv=1&lqi=&phdesc=YIieYjh-KdI&vet=12ahUKEwij-t2dkbOFAxX8RmwGHRE_DzwQ1YkKegQIGxAB..i&cs=0&um=1&ie=UTF-8&fb=1&gl=in&sa=X&geocode=Kav9gS0bRwk5Mcd-jAHHTNql&daddr=Upper+Rd,+Har+Ki+Pauri,+Haridwar,+Uttarakhand+249401",
        type: " Veg, Italian, South Indian",
        img: "data:image/jpeg;base64,"
    }]
}


export const GanverData: RiverData = {
    name: "",
    desc: "",
    background_url: "",
    air_weather: {
        aqi: 0,
        temp: 0,
        humidity: 0
    },
    water_weather: {
        ph: 0,
        temp: 0,
        turbuidity: 0
    },
    booking: {
        bus_url: "",
        train_url: "",
        cab_url: ""
    },
    locations: [{
        img: "",
        name: "",
        desc: "",
        map_url: ""
    },
    {
        img: "",
        name: "",
        desc: "",
        map_url: ""
    },
    {
        img: "",
        name: "",
        desc: "",
        map_url: ""
    }, {
        img: "",
        name: "",
        desc: "",
        map_url: ""
    },
    {
        img: "",
        name: "",
        desc: "",
        map_url: ""
    },
    {
        img: "",
        name: "",
        desc: "",
        map_url: ""
    },],
    hotels: [{
        name: "",
        img: "",
        address: "",
        booking_url: ""
    }, {
        name: "",
        img: "",
        address: "",
        booking_url: ""
    }, {
        name: "",
        img: "",
        address: "",
        booking_url: ""
    }],
    restorants: [{
        name: "",
        map_url: "",
        type: "",
        img: ""
    }, {
        name: "",
        map_url: "",
        type: "",
        img: ""
    }, {
        name: "",
        map_url: "",
        type: "",
        img: ""
    }]
}

export const GanData: RiverData = {
    name: "",
    desc: "",
    background_url: "",
    air_weather: {
        aqi: 0,
        temp: 0,
        humidity: 0
    },
    water_weather: {
        ph: 0,
        temp: 0,
        turbuidity: 0
    },
    booking: {
        bus_url: "",
        train_url: "",
        cab_url: ""
    },
    locations: [{
        img: "",
        name: "",
        desc: "",
        map_url: ""
    },
    {
        img: "",
        name: "",
        desc: "",
        map_url: ""
    },
    {
        img: "",
        name: "",
        desc: "",
        map_url: ""
    }, {
        img: "",
        name: "",
        desc: "",
        map_url: ""
    },
    {
        img: "",
        name: "",
        desc: "",
        map_url: ""
    },
    {
        img: "",
        name: "",
        desc: "",
        map_url: ""
    },],
    hotels: [{
        name: "",
        img: "",
        address: "",
        booking_url: ""
    }, {
        name: "",
        img: "",
        address: "",
        booking_url: ""
    }, {
        name: "",
        img: "",
        address: "",
        booking_url: ""
    }],
    restorants: [{
        name: "",
        map_url: "",
        type: "",
        img: ""
    }, {
        name: "",
        map_url: "",
        type: "",
        img: ""
    }, {
        name: "",
        map_url: "",
        type: "",
        img: ""
    }]
}



